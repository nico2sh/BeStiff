...
technique SimpleTexture
{
	pass Pass1
	{
		// MonoGame: pass states replace the whole rasterizer state, so the
		// cull mode must be explicit (XNA kept the device state, which was None).
		CullMode = None;
		StencilEnable = False;
...
technique PointLight_Shadow_Fast
{
	pass ShadowStencil
	{
		...
		AlphaBlendEnable = True;
		BlendOp = Add;
		// Write the hull's (opaque) alpha straight into the map: alpha = 1 in shadow.
		SrcBlend = One;
		DestBlend = Zero;
