...
namespace Be_Stiff
{
	public class EnemyFatUnarmed : Enemy
	{
		public EnemyFatUnarmed()
			: base(new Vector2(0.5f, 1.95f))
		{
		}
		...
		protected override void LoadEnemyType()
		{
			...
			skeleton = boneReader.CreateFromFile(GameElementsControl.ScreenManager.Content, "skeletons\\enemy\\fatguy\\fatguyskeleton", "skeletons\\enemy\\fatguy\\fatguyanimations", "sprites\\enemy\\fatguy\\", GameElementsControl.World, ConvertUnits.DisplayToSimUnitsRatio, bodyRect, GameElementsControl.IsLegacyLevel ? 1f : 2f, true, GameElementsControl.IsLegacyLevel ? 1f : 2f);
			...
			weapon = new Weapon[1];
			numberOfWeapons = 1;
			NoWeapon noWeapon = new NoWeapon(arm, skeleton.GetBoneByName("leftLoArm"));
			noWeapon.Load();
			noWeapon.Init(base.Name);
			noWeapon.SetPunchStrength(50f);
			noWeapon.SetDelayBetweenPunches(1100.0);
			weapon[0] = noWeapon;
			weapon[activeWeapon].SetArmAnimation();
			weapon[activeWeapon].WeaponRange = 2f;
