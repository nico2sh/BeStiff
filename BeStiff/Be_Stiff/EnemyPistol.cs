...
namespace Be_Stiff
{
	public class EnemyPistol : Enemy
	{
		protected override void LoadEnemyType()
		{
			...
			skeleton = boneReader.CreateFromFile(GameElementsControl.ScreenManager.Content, "skeletons\\enemy\\regularguy\\regularguyskeleton", "skeletons\\enemy\\regularguy\\regularguyanimations", "sprites\\enemy\\pistolarmed\\", GameElementsControl.World, ConvertUnits.DisplayToSimUnitsRatio, bodyRect, GameElementsControl.IsLegacyLevel ? 1f : 2f, true, GameElementsControl.IsLegacyLevel ? 1f : 2f);
			...
			weapon = new Weapon[1];
			numberOfWeapons = 1;
			Pistol pistol = new Pistol(arm, 0.5);
			pistol.Init(this);
			pistol.Load();
			weapon[0] = pistol;
