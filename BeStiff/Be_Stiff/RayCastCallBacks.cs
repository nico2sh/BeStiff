...
namespace Be_Stiff
{
	public static class RayCastCallBacks
	{
		...
		{
			if (Globals.CollidesWithHuman(f))
			{
				if (fr < fraction)
				{
					hitClosest = true;
		...
		{
			if (worldObjectsNum < 10 && Globals.Collides(f) && f.UserData is WorldObjectData worldObjectData && (f.CollidesWith & Category.Cat7) != Category.None && worldObjectData.Object.Name != ignoreWorldObjectName)
			{
				bool flag = false;
