...
public class FactoryEmployer<ProductType> : Employer where ProductType : class
{
	...
	private class ConcreteFactory : IAbstractFactory<ProductType>, IAbstractFactory
	{
		...
		{
			return (ProductType)Activator.CreateInstance(concreteType);
		}
	...
	public List<IAbstractFactory<ProductType>> Factories => employedFactories;

	/// <summary>Initializes a new FactoryEmployer</summary>
	public FactoryEmployer()
	{
		employedFactories = new List<IAbstractFactory<ProductType>>();
	}
	...
	{
		if (PluginHelper.HasDefaultConstructor(type) && typeof(ProductType).IsAssignableFrom(type))
		{
	...
	public override void Employ(Type type)
	{
		...
		{
			throw new MissingMethodException("Cannot employ type because it does not have a public default constructor");
		}
		if (!typeof(ProductType).IsAssignableFrom(type))
		{
			throw new InvalidCastException("Cannot employ type because it cannot be cast to the factory's product type");
		}
		if (type.ContainsGenericParameters)
		{
			throw new ArgumentException("Cannot employ type because it requires generic parameters", "type");
		}
		employedFactories.Add(new ConcreteFactory(type));
	}
}
