...
public struct UniScalar
{
	...
	public static readonly UniScalar Zero = default(UniScalar);
	...
	{
		return new UniScalar(scalar.Fraction + summand.Fraction, scalar.Offset + summand.Offset);
	}
	...
	{
		return new UniScalar(scalar.Fraction - subtrahend.Fraction, scalar.Offset - subtrahend.Offset);
	}
	...
	{
		return new UniScalar(scalar.Fraction / divisor.Fraction, scalar.Offset / divisor.Offset);
	}
	...
	{
		return new UniScalar(scalar.Fraction * factor.Fraction, scalar.Offset * factor.Offset);
	}
	...
	{
		return string.Format("{{{0}% {1}{2}}}", Fraction * 100f, (Offset >= 0f) ? "+" : string.Empty, Offset);
	}
}
